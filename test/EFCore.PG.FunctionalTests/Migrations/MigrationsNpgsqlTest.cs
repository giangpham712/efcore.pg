...
CREATE TABLE "People" (
    "Id" integer GENERATED BY DEFAULT AS IDENTITY,
    "Sum" text GENERATED ALWAYS AS (("X" + "Y")::text) STORED,
    "X" integer NOT NULL,
    "Y" integer NOT NULL,
    CONSTRAINT "PK_People" PRIMARY KEY ("Id")
);
...
""");
    ...
    public override async Task Move_table()
    {
        ...
        AssertSql(
            // CockroachDB doesn't support IF statement, https://github.com/cockroachdb/cockroach/issues/110080
            TestEnvironment.IsCockroachDB ?
"""
...
""");
    ...
    public override async Task Add_column_with_computedSql(bool? stored)
    {
        ...
        AssertSql(TestEnvironment.IsCockroachDB ?
            @"ALTER TABLE ""People"" ADD ""Sum"" text GENERATED ALWAYS AS ((""X"" + ""Y"")::text) STORED;" :
...
""");
    ...
    public override async Task Alter_column_make_computed(bool? stored)
    {
        ...
        AssertSql(
            ...
            //
            TestEnvironment.IsCockroachDB ?
                @"ALTER TABLE ""People"" ADD ""Sum"" integer GENERATED ALWAYS AS ((""X"" + ""Y"")::integer) STORED NOT NULL;" :
    ...
    public virtual async Task Create_index_with_include_and_filter()
    {
        await Test(
            ...
            model =>
            {
                var table = Assert.Single(model.Tables);
                var index = Assert.Single(table.Indexes);
                Assert.Contains(@"""Name"" IS NOT NULL", index.Filter);
                Assert.Single(index.Columns);
                Assert.Contains(table.Columns.Single(c => c.Name == "Name"), index.Columns);
...
""",
    ...
    public override async Task Drop_unique_constraint()
    {
        ...
        AssertSql(
            TestEnvironment.IsCockroachDB ?
                @"DROP INDEX ""AK_People_AlternateKeyColumn"" CASCADE;" :
...
""");
    ...
    public virtual async Task Add_column_generated_tsvector_over_text()
    {
        ...
        AssertSql(TestEnvironment.IsCockroachDB ?
            @"ALTER TABLE ""Blogs"" ADD ""SearchColumn"" tsvector GENERATED ALWAYS AS ((to_tsvector('english', ""TextColumn""))::tsvector) STORED;" :
        ...
        public override DbContextOptionsBuilder AddOptions(DbContextOptionsBuilder builder)
        {
            new NpgsqlDbContextOptionsBuilder(base.AddOptions(builder)
                    ...
                    .ConfigureWarnings(
                        w =>
                        {
                            w.Ignore(NpgsqlEfEventId.ExpressionIndexSkippedWarning);

                            // CockroachDB automatically creates primary key if not specified
                            if (TestEnvironment.IsCockroachDB)
                            {
                                w.Ignore(NpgsqlEfEventId.UnsupportedColumnConstraintSkippedWarning);
                            }
                        }))
